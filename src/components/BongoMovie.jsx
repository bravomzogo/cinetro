...
export default function BongoMovie() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/bongomovies/${id}/`);
        if (!response.ok) throw new Error('Failed to fetch Bongo movie');
        const data = await response.json();
        if (data.trailer_url && !data.trailer_url.startsWith('http')) {
          data.trailer_url = `https://www.youtube.com/watch?v=${data.trailer_url}`;
        }
        setMovie(data);
      } catch (err) {
  ...
  if (error) return (
    <div className="flex h-screen justify-center items-center bg-gray-900">
      <div className="text-center p-6 bg-red-900/50 rounded-xl">
        ...
        <h2 className="text-2xl font-bold text-red-400">Error Loading Bongo Movie</h2>
  ...
  if (!movie) return (
    <div className="flex h-screen justify-center items-center bg-gray-900">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-300">Bongo Movie Not Found</h2>
        <p className="text-gray-500 mt-2">The requested movie could not be loaded</p>
        <button 
          onClick={() => navigate('/bongomovies')}
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white font-sans overflow-hidden">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <p>
                      ...
                      <span className="ml-2">{movie.duration ? `${Math.floor(movie.duration / 60)}h ${movie.duration % 60}m` : 'Not specified'}</span>
                    </p>
                    <p>
      ...
      <AnimatePresence>
        {showTrailer && (
          <Trailer 
            youtubeId={movie.trailer_url.includes('youtube.com') ? movie.trailer_url.split('v=')[1] : movie.trailer_url}
